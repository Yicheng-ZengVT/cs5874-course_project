...
def inference(_class_, pars):
    ...
    checkpoint_fusion = ckp.get('fusion_mode', None)
    if checkpoint_fusion is not None:
        print(f'Checkpoint saved with fusion_mode={checkpoint_fusion}')

    print(f'Running inference with fusion_mode={pars.fusion}')
    ...
    auroc_px, auroc_sp, aupro_px = evaluation_multi_proj(
        ...
        test_dataloader,
        device,
        fusion_mode=pars.fusion
    )

    print(
        '{}: Fusion {}, Sample Auroc: {:.4f}, Pixel Auroc:{:.4f}, Pixel Aupro: {:.4f}'.format(
            _class_, pars.fusion, auroc_sp, auroc_px, aupro_px
        )
    )
...
if __name__ == '__main__':
    ...
    for c in pars.classes:
        ...
        metrics['fusion_mode'].append(pars.fusion)
