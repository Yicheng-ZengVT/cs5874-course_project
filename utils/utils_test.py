...
def compute_anomaly_map_list(fs_list, ft_list, out_size=224):
    """
    Compute one anomaly map per feature level and return the list only
    """
    a_map_list = []
...
def fuse_anomaly_maps(a_map_list, fusion_mode='sum'):
    """
    ...
    Supported modes:
        - 'sum'  : sum of maps
        - 'mean' : mean of maps
        - 'max'  : elementwise max across maps
        - 'mul'  : elementwise product (kept for backward compatibility)
        - 'a'    : alias for 'sum' to match old repo behavior
    """
    if len(a_map_list) == 0:
...
def cal_anomaly_map(fs_list, ft_list, out_size=224, amap_mode='sum'):
    ...
    Returns:
        anomaly_map: fused anomaly map
        a_map_list: list of per-layer anomaly maps
    """
    a_map_list = compute_anomaly_map_list(fs_list, ft_list, out_size=out_size)
    anomaly_map = fuse_anomaly_maps(a_map_list, fusion_mode=amap_mode)
    return anomaly_map, a_map_list
...
def evaluation_multi_proj(encoder, proj, bn, decoder, dataloader, device, fusion_mode='sum'):
    encoder.eval()
    proj.eval()
    bn.eval()
    decoder.eval()
